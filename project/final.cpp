...
bool validateCredentials() {
    // username 'rounak' and password 'admin'

    return username == "rounak" && password == "admin";
}
...

{
...

  {
    ...
    if (strcmp(bus[y].bus_seat[bus_seat/4][(bus_seat%4)-1], "Empty")==0)
      ...
      cout<<"**********************************************************\n";
      cout<<"         The Seat Number. "<<bus_seat<<" is Already Reserved.         \n";
      cout<<"**********************************************************\n";
      }
    ...
    bus[t].empty();
  cout << "----------------------------------------------------";
  cout<<"\n     Your's Seat number "<<bus_seat<<" is reserved for "<<bus[y].bus_seat[bus_seat/4][(bus_seat%4)-1]<<"\t\t\t     "<<endl;
  cout << "----------------------------------------------------\n";
...
void a::cancelReservation() {
    char bus_number[5];
    cout << "Enter Bus Number: ";
    cin >> bus_number;

    int bus_index;
    for (bus_index = 0; bus_index <= t; bus_index++) {
        if (strcmp(bus[bus_index].bus_number, bus_number) == 0) {
            break;
        }
    }
    ...
    if (strcmp(bus[bus_index].bus_seat[row][col], "Empty") != 0) {
        strcpy(bus[bus_index].bus_seat[row][col], "Empty");
        cout << "Reservation for Seat Number " << seat_number << " has been canceled." << endl;
    } else {
        cout << "Seat Number " << seat_number << " is already empty." << endl;
    }
}
...

{
...

  {
    ...
    case 4:  bus[0].cancelReservation();
      break;
